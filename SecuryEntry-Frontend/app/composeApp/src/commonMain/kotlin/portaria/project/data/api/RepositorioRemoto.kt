...
class RepositorioRemoto(private val client: HttpClient = provideHttpClient()) {
    ...
    suspend fun createReservaVaga(reserva: ReservaVaga): String = call {
        client.post("$base/reservas-vagas") {
            contentType(ContentType.Application.Json)
            setBody(reserva)
        }.body()
    }

    suspend fun getReservasVagas(apartmentId: String? = null): List<ReservaVaga> = call {
        client.get("$base/reservas-vagas") {
            if (!apartmentId.isNullOrBlank()) {
                parameter("apartmentId", apartmentId)
            }
        }.body()
    ...
    suspend fun getReservaVagaById(id: String): ReservaVaga = call {
        client.get("$base/reservas-vagas/$id").body()
    }

    suspend fun updateReservaVaga(id: String, reserva: ReservaVaga) = call {
        client.put("$base/reservas-vagas/$id") {
            contentType(ContentType.Application.Json)
            setBody(reserva)
        }.bodyAsText()
    }

    suspend fun deleteReservaVaga(id: String) = call {
        client.delete("$base/reservas-vagas/$id").bodyAsText()
    }

    suspend fun createOcorrencia(ocorrencia: Ocorrencia): String = call {
        client.post("$base/ocorrencias") {
            contentType(ContentType.Application.Json)
            setBody(ocorrencia)
        }.body()
    }

    suspend fun getOcorrencias(apartmentId: String? = null): List<Ocorrencia> = call {
        client.get("$base/ocorrencias") {
            if (!apartmentId.isNullOrBlank()) {
                parameter("apartmentId", apartmentId)
            }
        }.body()
    ...
    suspend fun getOcorrenciaById(id: String): Ocorrencia = call {
        client.get("$base/ocorrencias/$id").body()
    }

    suspend fun updateOcorrencia(id: String, ocorrencia: Ocorrencia) = call {
        client.put("$base/ocorrencias/$id") {
            contentType(ContentType.Application.Json)
            setBody(ocorrencia)
        }.bodyAsText()
    }

    suspend fun deleteOcorrencia(id: String) = call {
        client.delete("$base/ocorrencias/$id").bodyAsText()
    }
